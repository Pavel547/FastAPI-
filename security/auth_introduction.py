...
async def get_current_user(token: Annotated[str, Depends(oauth2_sheme)]):
    credential_eception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Invalid username or password",
        headers={"WWW-Authenticete": "Bearer"}
    )
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=ALGORITHM)
        username: str = payload.get("sub")
        if username is None:
            return credential_eception
...
async def login_for_acces_token(
    ...
    acces_token_expire = timedelta(minutes=ACCESS_TOKEM_EXPIRE_MINUTE)
